...
class MovieAnimationAdapter(private var onClick: (Movie) -> Unit) :
    ListAdapter<Movie, MovieAnimationAdapter.MovieViewHolder>(MovieDiffCallback()) {

    inner class MovieViewHolder(val binding: MovieItemListBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(currentMovie: Movie) {
            Glide.with(binding.root).load(currentMovie.coverImg).override(130, 130).skipMemoryCache(false)

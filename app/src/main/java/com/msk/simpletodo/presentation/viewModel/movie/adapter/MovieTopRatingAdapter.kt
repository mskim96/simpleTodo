...
class MovieTopRatingAdapter(private val onClick: (Movie) -> Unit) :
    ListAdapter<Movie, MovieTopRatingAdapter.MovieViewHolder>(MovieDiffCallback()) {

    inner class MovieViewHolder(val binding: MovieItemListBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(currentMovie: Movie) {
            Glide.with(binding.root).load(currentMovie.coverImg).override(130, 200).skipMemoryCache(false)
